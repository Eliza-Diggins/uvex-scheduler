...
class VanVelzenTDESED(ComposedSpectralModel):
    ...
    with :math:`t_\mathrm{peak} = 5\sigma` (:class:`~uvex_transient_toolkit.models.lightcurves.generic.GREDLightcurve`'s
    ...
    T^4)` factor is exactly :class:`~uvex_transient_toolkit.models.spectra.thermal.BlackbodySpectrum`'s
    normalized shape, :math:`\int_0^\infty S(\nu, T)\,d\nu = 1`), so
    :math:`L_0` here is literally :math:`L_\mathrm{bol}(t_\mathrm{peak})`.
    Composed from :class:`~uvex_transient_toolkit.models.lightcurves.generic.GREDLightcurve`
    (already bolometric -- :math:`L_0 \cdot \ell(t)`) and
    :class:`~uvex_transient_toolkit.models.spectra.thermal.BlackbodySpectrum`; see
